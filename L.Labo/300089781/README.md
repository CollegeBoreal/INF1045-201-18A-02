...
<br>
Pour terminer avec le client, il faut seulement faire la commande:
```
...
:exclamation: Comme vous pouvez voir, la dernière ligne #9 ci-dessus pour le client et la dernière ligne #10 ci-dessous pour le serveur montre que le tunnel a été créé! Pour voir si nous pouvons nous connecter du client au serveur, il suffit de faire un ping sur le terminal client cb-dev avec l'adresse LOCAL du serveur az-cb-prod que le VPN a créé pour le tunnel:
